...
void CBase64::AllocEncode(unsigned long nSize)
{
  if(m_nEBufLen < nSize)
  {
    if(m_pEBuffer != NULL)
...
{
  if(szMessage != NULL)
    CBase64::Encode((const unsigned char*)szMessage, lstrlenA(szMessage));
}

void CBase64::Decode(const unsigned char* pBuffer, unsigned long dwBufLen)
{
  if(!CBase64::m_Init)
    _Init();
...
{
  if(szMessage != NULL)
    CBase64::Decode((const unsigned char*)szMessage, lstrlenA(szMessage));
}

unsigned long CBase64::_DecodeToBuffer(const TempBucket &Decode, unsigned char* pBuffer)
{
  TempBucket  Data;
...
BOOL CBase64::_IsBadMimeChar(unsigned char nData)
{
  switch(nData)
  {
  case '\r': case '\n': case '\t': case ' ' :
