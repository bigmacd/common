...
{
#ifdef _DEBUG
    cout << "in NTService constructor" << endl;
#endif
...
BOOL CNTService::ParseStandardArgs(int argc, char* argv[])
{
    ...
    if (_stricmp(argv[1], "-v") == 0) {
        ...
        printf("The service has%sbeen installed.\n",
               IsInstalled() ? " " : " not ");
        ...
        if (IsInstalled()) {
            ...
            if (Install()) {
                ...
                printf("%s failed to install. Error is %d.\n", m_szServiceName, GetLastError());
            }
        }
        return TRUE; // say we processed the argument
        ...
        if (!IsInstalled()) {
            printf("%s has not been installed\n", m_szServiceName);
        } else {
            ...
            if (Uninstall()) {
                ...
                printf("Could not remove %s. Error is %d.\n", m_szServiceName, GetLastError());
            }
        }
        return TRUE; // say we processed the argument
...
{
#ifdef _DEBUG
    cout << "in NTService StartService" << endl;
#endif
...
#ifdef _DEBUG
    cout << "leaving NTService StartService" << endl;
#endif
    return b;
}
...
#ifdef _DEBUG
    cout << "in NTService Initialize" << endl;
#endif
...
#ifdef _DEBUG
    cout << "NTService:OnInit failed!" << endl;
#endif
        return FALSE;    
    }
    
#ifdef _DEBUG
    cout << "NTService: setting status to running" << endl;
#endif
...
void 
CNTService::SetDescription(const char* description)
{
  // add a description to the service entry
  char szKey[256];
  HKEY hKey = NULL;
  strcpy_s(szKey, "SYSTEM\\CurrentControlSet\\Services\\");
  strcat_s(szKey, m_szServiceName);
